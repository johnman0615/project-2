...
((SELECT user_id FROM users WHERE email='robertbrown@example.com'), 'Brown Realty', 4.8),
((SELECT user_id FROM users WHERE email='emilyjohnson@example.com'), 'Johnson & Co. Real Estate', 4.6);
...
('A one story Villa includes 2 Amazing bedrooms, with an en-suite bathroom in the Primary bedroom.', 259900.00, '112 Coventry Ln', 'Haines City', 'FL', '33844', 'house', 2, 2, 1226, 
 (SELECT agent_id FROM agents WHERE agency_name='Brown Realty'), 
 (SELECT user_id FROM users WHERE email='johndoe@example.com'), 'available'),
...
('Contemporary home in Crystal Beach is a waterfront masterpiece.', 2690000.00, '812 Point Seaside Dr', 'Crystal Beach', 'FL', '34681', 'house', 8, 7, 8908, 
 (SELECT agent_id FROM agents WHERE agency_name='Johnson & Co. Real Estate'), 
 (SELECT user_id FROM users WHERE email='janesmith@example.com'), 'available'),

('Townhouse with low maintenance.', 340000.00, '2414 Hounds Trl', 'Palm Harbor', 'FL', '34683', 'townhouse', 2, 3, 1260, 
 (SELECT agent_id FROM agents WHERE agency_name='Brown Realty'), 
 (SELECT user_id FROM users WHERE email='johndoe@example.com'), 'pending'),
...
('Home nestled on a desirable corner lot in a friendly community.', 285000.00, '4505 Little River Ln', 'Fort Myers', 'FL', '33905', 'house', 2, 2, 1486, 
 (SELECT agent_id FROM agents WHERE agency_name='Johnson & Co. Real Estate'), 
 (SELECT user_id FROM users WHERE email='janesmith@example.com'), 'sold'),
...
('Perfect blend of luxury and comfort.', 850000.00, '16200 Ivy Lake Dr', 'Odessa', 'FL', '33556', 'house', 4, 3, 3540, 
 (SELECT agent_id FROM agents WHERE agency_name='Johnson & Co. Real Estate'), 
 (SELECT user_id FROM users WHERE email='janesmith@example.com'), 'pending'),
...
('One story block construction home with 2 car garage.', 799000.00, '612 Addison Dr NE', 'Saint Petersburg', 'FL', '33716', 'house', 4, 3, 2754, 
 (SELECT agent_id FROM agents WHERE agency_name='Johnson & Co. Real Estate'), 
 (SELECT user_id FROM users WHERE email='janesmith@example.com'), 'available');

('Make this beautifully updated 3-bed, 2-bath home blends modern style with comfort home yours.', 739000.00, '7820 SW 33rd Ter', 'Miami', 'FL', '33155', 'house', 3, 2, 1271, 
 (SELECT agent_id FROM agents WHERE agency_name='Johnson & Co. Real Estate'), 
 (SELECT user_id FROM users WHERE email='janesmith@example.com'), 'available');
